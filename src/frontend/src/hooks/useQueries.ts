...
export function useGetOnlineUsers() {
  ...
  return useQuery<string[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      try {
        const principals = await actor.getOnlineUsers();
...
export function useGetNotifications() {
  ...
  return useQuery<Notification[]>({
    ...
    queryFn: async () => {
      if (!actor) return [];
      // Fetch up to 100 notifications for tab-filtered views
      const result = await actor.getNotifications(BigInt(100), BigInt(0));
...
export function useGetNotificationsPaginated(offset: number, limit = 6) {
  ...
  return useQuery<{ notifications: Notification[]; total: bigint }>({
    ...
    queryFn: async () => {
      if (!actor) return { notifications: [], total: BigInt(0) };
...
export function useMarkNotificationAsRead() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["notificationsPaginated"] });
