// Migration: add `username : Text` to the Fair Spin `SpinRecord` type so the
// global game history can show every player's name without joining the
// profiles map at read time. OldActor matches the previous migration's
// NewActor exactly (52 stable fields, SpinRecord without username). NewActor
// is identical except the inlined SpinRecord type carries the new username
// field. The migration body maps every existing field through unchanged and
// rewrites the per-player gameHistory arrays so each historical SpinRecord
// gains `username = "unknown"` (the original player identity cannot be
// resolved at upgrade time, so a placeholder is used; new spins capture the
// real username at spin time).
//
...
module {
  ...
  // Old SpinRecord shape (deployed, without username).
  type OldSpinRecord = {
    id : Nat;
    player : Principal;
    stake : Float;
  ...
  // New SpinRecord shape (with username).
  type NewSpinRecord = {
    id : Nat;
    player : Principal;
    username : Text;
  ...
  type OldGameState = {
    ...
    var gameHistory : Map.Map<Principal, [OldSpinRecord]>;
    var nextSpinId : Nat;
  };
  ...
  type NewGameState = {
    ...
    var playerParticipation : Map.Map<Principal, Float>;
    var lastSpinTime : Map.Map<Principal, Int>;
    var gameHistory : Map.Map<Principal, [NewSpinRecord]>;
    var nextSpinId : Nat;
  };
  ...
  type OldActor = {
    ...
    var gameState : OldGameState;
  };

  // ── NewActor: same 52 stable fields; gameState uses the new SpinRecord ──────
  type NewActor = {
    accessControlState : AccessControlState;
    adminUsername : Text;
    var blockListMap : Map.Map<Principal, [Principal]>;
  ...
  // Rewrite a player's history array so every OldSpinRecord gains the new
  // username field. Historical usernames cannot be resolved at upgrade time,
  // so a placeholder is used; new spins capture the real username.
  func migrateHistory(records : [OldSpinRecord]) : [NewSpinRecord] {
    records.map(
      func(r : OldSpinRecord) : NewSpinRecord {
        {
          id = r.id;
          player = r.player;
          username = "unknown";
  ...
  public func migration(old : OldActor) : NewActor {
    // Build the new gameState from the old one, rewriting gameHistory.
    let newGameHistory = Map.empty<Principal, [NewSpinRecord]>();
    for ((player, records) in old.gameState.gameHistory.entries()) {
      newGameHistory.add(player, migrateHistory(records));
    };
    {
      ...
      var gameState = {
        var playerPool = old.gameState.playerPool;
        var playerParticipation = old.gameState.playerParticipation;
        var lastSpinTime = old.gameState.lastSpinTime;
        var gameHistory = newGameHistory;
        var nextSpinId = old.gameState.nextSpinId;
      };
    };
  };
};
